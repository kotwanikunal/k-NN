...
public class HalfFloatIndexIT extends KNNRestTestCase {
     ...
     * {@link org.opensearch.knn.index.codec.KNN1040Codec.KNN1040ScalarQuantizedVectorScorer}. That scorer maps
     * COSINE onto the {@code SQ_IP} native kernel, which applies the max-inner-product transform
     * {@code 1 + dot} rather than cosine's {@code (1 + dot) / 2} - this branch has no {@code SQ_COSINE}
     * kernel (it is main-only work that predates the half_float commit). Vectors are L2-normalized on
     * write and the query is normalized too, so {@code dot} is still exactly cosine; only the final
     * scale differs, and it differs identically for float SQ 1-bit on this branch. Ranking is
     * unaffected because the transform is monotonic.
     */
    @SneakyThrows
    public void testHalfFloatFlatIndex_cosineSpace_scoresMatchCosine() {
        assertCosineScoresMatch(buildHalfFloatMapping("cosinesimil"), true);
    }
