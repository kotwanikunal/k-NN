...
public class KNN1040HalfFloatVectorScorerTests extends KNNTestCase {
    ...
    /**
     * COSINE has no native FP16 kernel on this branch - {@code NativeSimilarityFunctionType} carries only
     * {@code FP16_MAXIMUM_INNER_PRODUCT}, {@code FP16_L2}, {@code SQ_IP} and {@code SQ_L2}, so
     * {@link org.opensearch.knn.index.codec.scorer.NativeEngines990KnnVectorsScorer#getNativeFunctionType}
     * returns {@code null} for it. Cosine therefore takes the fallback tier regardless of SIMD support, which
     * still scores the FP16 data correctly (it decodes via {@code DefaultFlatVectorScorer}) - just not
     * decode-free. Upstream asserts the native tier here because it carries an additional {@code FP16_COSINE}
     * kernel that is not part of the half_float feature being ported.
     */
    @SneakyThrows
    public void testGetRandomVectorScorerSupplier_cosineWithSimdSupported_usesFallbackTierAbsentNativeCosineKernel() {
