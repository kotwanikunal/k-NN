...
public class JNIService {
    ...
    public static void createIndex(
        ...
        if (KNNEngine.FAISS == knnEngine) {
            if (parameters.get(KNNConstants.INDEX_DESCRIPTION_PARAMETER).toString().startsWith(FAISS_BINARY_INDEX_PREFIX)) {
    ...
    public static long loadIndex(String indexPath, Map<String, Object> parameters, KNNEngine knnEngine) {
        ...
        if (KNNEngine.FAISS == knnEngine) {
            //TODO heemin pass index description in parameter
            if (parameters.get(KNNConstants.INDEX_DESCRIPTION_PARAMETER).toString().startsWith(FAISS_BINARY_INDEX_PREFIX)) {
    ...
    public static KNNQueryResult[] queryBinaryIndex(
        ...
        if (KNNEngine.FAISS == knnEngine) {
            if (ArrayUtils.isEmpty(filteredIds) == false || parentIds != null) {
                // Faiss library does not support search parameter for binary index which is required to pass filteredIds and parentIds.
                throw new IllegalArgumentException("QueryBinaryIndex does not support filteredIds and parentIds");
            }
            return FaissService.queryBinaryIndexWithFilter(indexPointer, queryVector, k, ArrayUtils.isEmpty(filteredIds) ? null : filteredIds, filterIdsType, parentIds);
        }
